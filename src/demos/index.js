// demo router
export {default as DemoRouter} from './demoRouter'
// demos
export {default as CameraDemo} from './camera-demo'
export {default as InputDemo} from './input-demo'
export {default as EinDemo} from './ein'

export {default as ScreenManager} from './screen-manager'
export {default as GamepadDemo} from './gamepad'
export {default as Box2dDemo} from './box2d'
export {default as FirebaseDemo} from './firebase-demo'
export {default as GyroDemo} from './gyro-demo/gyro'

export {default as SimpleInitDemo} from './simple-init'
